// A colour replacement/fill tool 
function FloodFillTool(){
    ...
    this.genHSL = function(colour){
        colour = colour.slice(4,colour.length - 1).split(",");
        for(let i = 0; i < colour.length; i++) {
            colour[i] = parseInt(colour[i]);
        }
//        colour.push(255);
        return this.rgbHSL(colour);
    };
    this.indexInner = function(array,startColour,colour){
        ...
        for(let i = 0; i < array.length - 1; i++) {
            ...
            if(!this.checkColour(p1[0]+1,p1[1],startColour)){
                continue;
            }
    ...
    this.replaceColour = function(array,colour,startColour) {
        // replace colour
	...
	this.draw = function(){
        ...
        if(mouseIsPressed){
            // check if mouse location is within the canvas
            if(mouseX >= 0 && mouseX <= width && 
               mouseY >= 0 && mouseY <= height) {
                // set/reset this.points to an empty array
                this.points = [];
                ...
                let startColour = this.getColour(x,y);
                ...
                // select shape
                this.selectColour(this.points,startColour,colour,x,y);
                // replace colour
                this.replaceColour(this.points,colour,startColour);
            }	
        }
	};
    ...
    this.populateOptions = function(){
        $(".options").html("<button id='hButton'>Fill</button>");
		//click handler
		$("#hButton").on("click", function(){
            // if Colour Replacement is on, switch to Fill
            if(self.hRange == 10){ 
                self.hRange = 1;
                self.sRange = 10;
                self.lRange = 10;
                $(this).text('Colour Replacement');
            }
            // if Fill is on, switch to Colour Replacement
            else{
                self.hRange = 10;
                self.sRange = 100;
                self.lRange = 100;
